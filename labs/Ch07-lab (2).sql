...
dense_rank() over (partition by custid order by qty) as denserank
FROM dbo.Orders
...
SELECT custid, orderid, qty,
              qty -lag(qty) over(partition by custid
                                                       ORDER by orderdate)  as QTY,
              qty -lead(qty) over(partition by custid order by orderdate) as b
FROM dbo.orders
...
SELECT empid,
              count(orderid) over (partition by empid, year(2014)) as year2014,
              count(orderid) over (partition by empid, year(2015)) as year2015
              
FROM dbo.orders
GROUP by empid, orderid
